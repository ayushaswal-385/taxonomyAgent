---
name: mcat-taxonomy-pipeline-v6.0
description: >
...
# MCAT Taxonomy Pipeline v6.0 — Complete Skill Reference
...
```
Pre-Pipeline (run ONCE across the full mcat_list.csv batch before any pipeline run):
  catalogue_preprocessor.py
    Inputs: all_mcats_indiamart.csv, taxonomy_tree.csv, related_mcats_overlap.csv,
            mcat_related_categories.csv, mcat_list.csv
    Output: agent4_input/{mcat_id}/agent4_input.json — one file per MCAT
    Zero token cost — pure Python, no API calls.
    Produces per MCAT: taxonomy_context + full pre-built candidate pool (Tier 1a/1b/2/3/4)
    with overlap data attached, mcat_level and listing_count on every candidate,
    and family_context_pairs for Source C reasoning.
    Rerun only when source data changes. Never runs inside the per-MCAT loop.
...
Per-MCAT loop ─────────────────────────────────────────────────────────
  ...
  Wave 1 (sequential):
    Agent 3  — Context Extraction + Market Research + Listing Quality Check
               Inputs: page URL, google KWs, internal KWs,
                       ...
                       taxonomy_context from agent4_input.json (passed by orchestrator)

  Wave 2 (parallel — all consume Agent 3 vision_output):
    Agent 4  — MCAT Relationship Mapper
               Inputs: vision_output (Agent 3 — includes taxonomy_context),
                       agent4_input.json for THIS MCAT (pre-built by preprocessor)
               No raw CSV files. Candidate pool, overlap data, and family context
               are fully pre-assembled. Agent 4 classifies only.
    ...
    Agent 8  — Category Nature
               Uses taxonomy_context.mcat_level and listing_count from vision_output
  ...
  Wave 3a (sequential — must complete before Wave 3b):
    Agent 12 — Name Generator
               Uses taxonomy_context.siblings (full sibling name set) from vision_output
               Produces alias_collision_merge_signals + naming_inconsistency_flag
               → both consumed by Agent 14

  Wave 3b (sequential):
    Agent 14 — Pre-Cleaning Structural Verdict
               Uses taxonomy_context from vision_output
               Produces reposition_signal, intermediate_node_signal,
               naming_inconsistency_flag (from Agent 12) in structural_verdict
...
### Preprocessor Inputs (consumed by `catalogue_preprocessor.py` before pipeline runs)

| # | Input File | Format | Key Columns |
|---|---|---|---|
| P1 | `all_mcats_indiamart.csv` | CSV | `glcat_mcat_id, glcat_mcat_name` — full ~98K MCAT catalogue |
| P2 | `taxonomy_tree.csv` | CSV | `MCAT ID, MCAT Name, MCAT Level, CAT ID, CAT Name, Group ID, Group Name, PMCAT ID, PMCAT Name, PMCAT Level, Listing Count` |
| P3 | `related_mcats_overlap.csv` | CSV | `MCAT_1, MCAT_2, MCAT_1_Products, MCAT_2_Products, Common_Product, product_overlap_pct (M1.M2)/M1, product_overlap_pct (M1.M2)/M2, MCAT_1_Supplier, MCAT_2_Supplier, Common_Suppliers, supplier_overlap_pct (M1.M2)/M1, supplier_overlap_pct (M1.M2)/M2` |
| P4 | `mcat_related_categories.csv` | CSV | `glcat_mcat_id, glcat_mcat_name, related_mcat_name` — existing platform-level related MCAT links |
| P5 | `mcat_list.csv` | CSV | `mcat_id, mcat_name, mcat_url` — batch to process |

None of P1–P5 are ever passed to any agent. The preprocessor produces one `agent4_input.json` per MCAT from these files. Rerun the preprocessor whenever source data changes.

### Pipeline Inputs (consumed by agents during the per-MCAT loop)

| # | Input File | Format | Key Columns | Used By |
|---|---|---|---|---|
| 1 | `mcat_list.csv` | CSV | `mcat_id, mcat_name, mcat_url` | Orchestrator (iterates per row) |
...
| 4 | `agent4_input/{mcat_id}/agent4_input.json` | JSON | Pre-built by preprocessor. Contains taxonomy_context, full candidate pool with all tiers, overlap data per candidate, mcat_level and listing_count on every candidate, family_context_pairs. | Agent 4; taxonomy_context passed to all Wave 2+ agents via vision_output |

**Note on Input 4:** `agent4_input.json` replaces the three files Agent 4 previously read at runtime (`mcat_related_categories.csv`, `related_mcats_overlap.csv`, `preprocessed_candidates.csv`). Agent 4 no longer touches any raw CSV files. The orchestrator loads `agent4_input.json` for the current MCAT and passes `taxonomy_context` to Agent 3 at the start of Wave 1, which carries it forward in `vision_output` to all downstream agents.

---

## Preprocessor — `catalogue_preprocessor.py`

**Runs:** Once before the pipeline, across the full batch. NOT inside the per-MCAT loop.
**Purpose:** Builds one `agent4_input/{mcat_id}/agent4_input.json` per MCAT from five source files. Zero API calls. Pure Python. Rerun whenever source data changes.

**Startup (once):** Load all five source files into memory.
**Per-MCAT loop:** For each row in `mcat_list.csv`, extract and write `agent4_input.json`.

---

### Candidate Tiers

| Tier | Source | Inclusion rule | Notes |
|---|---|---|---|
| Tier 1a — Direct siblings | `taxonomy_tree.csv` | All MCATss sharing any PMCAT ID with root | Guaranteed regardless of keyword score. Both PMCAT parents included for multi-PMCAT MCATss. |
| Tier 1b — Indirect siblings | `taxonomy_tree.csv` | Children of root's immediate PMCAT's parent PMCAT — **only** when that parent is itself a PMCAT (not a CAT) | Expands to grandparent family. If immediate PMCAT's parent is a CAT, no indirect siblings. |
| 2 — Overlap confirmed | `related_mcats_overlap.csv` | All rows where root MCAT appears as MCAT_1 or MCAT_2 | Overlap metrics attached. Net-new after dedup with Tier 1. |
| 3 — Platform links | `mcat_related_categories.csv` | All `related_mcat_name` values for root MCAT | All platform-assigned links included for audit. Net-new after dedup. |
| 4 — Semantic | `all_mcats_indiamart.csv` | All MCATss scoring ≥ 0.7 via keyword + fuzzy + synonym | No cap. Full token match required (not substring). Net-new after dedup with Tiers 1–3. |

Deduplication by `mcat_id` across all tiers. Each candidate records `source_flags` (list of all tiers it appeared in) and `source_tier_primary` (first tier that found it).

---

### Semantic Scoring (Tier 4)

**Step 1 — Tokenise root name:** Split on whitespace/punctuation, strip stop words.
`"FRP Swimming Pools"` → `["FRP", "Swimming", "Pools"]`

**Step 2 — Full token match scoring:**
Tokens must appear as standalone tokens in the candidate name — not as substrings.
`"dynamo"` matches `"Dynamo Flashlight"` ✓ but NOT `"Dynamometer"` ✗

| Condition | Score |
|---|---|
| Exact full-name match | 1.0 |
| All root tokens present as full tokens | 0.9 |
| Majority of root tokens present | 0.7 |
| Single meaningful root token present | 0.4 |
| No full token overlap | 0.0 — discard |

**Step 3 — Fuzzy similarity:** `rapidfuzz token_sort_ratio` weighted 0.3×. Added to keyword score.

**Step 4 — Synonym boost:** +0.2 for synonym matches. Single-word synonyms: full token match required. Multi-word synonyms (e.g. "swimming pool"): substring match.

Synonym map (extend as catalogue grows): `FRP ↔ Fiberglass/Fibreglass/GRP`, `Pool ↔ Swimming Pool/Pools`, `Prefab ↔ Readymade/Modular/Portable`, `HDPE ↔ High Density Polyethylene`, `MS ↔ Mild Steel`, `GI ↔ Galvanized Iron`, `SS ↔ Stainless Steel`, `AC ↔ Air Conditioner`, and others.

**Step 5 — Threshold:** Keep only candidates with final score ≥ **0.7**. No cap.

**Step 6 — Pre-label:**
`pre_relevance`: `high` (score ≥ 0.7 or synonym match), `medium` (0.4–0.69), `low` (0.3–0.39)
`pre_relationship`: `likely_parent` (root name is substring of candidate), `likely_child` (candidate is substring of root), `likely_sibling` (primary noun token shared), `unknown`

---

### Per-Candidate Tree Enrichment

For every candidate regardless of tier, the preprocessor looks up `taxonomy_tree.csv` and attaches:
- `mcat_level` — tree depth of this candidate (None if not found in tree)
- `listing_count` — active listing count (None if not found in tree)

These fields are present on every candidate entry in `agent4_input.json`.

---

### `agent4_input.json` Structure

```json
{
  "root_mcat_id":   "30860",
  "root_mcat_name": "Inflatable Swimming Pool",

  "taxonomy_context": {
    "mcat_id": "30860", "mcat_name": "Inflatable Swimming Pool",
    "mcat_level": 4, "listing_count": "1268",
    "full_path": "Sports Goods... > Swimming Pool & Water Sports... > Swimming Pools > Inflatable Swimming Pool",
    "is_orphan": false,
    "group":  {"id": "36",    "name": "Sports Goods, Games, Toys & Accessories"},
    "subcat": {"id": "530",   "name": "Swimming Pool & Water Sports Goods and Accessories"},
    "parent": {"id": "39182", "name": "Swimming Pools", "level": "3"},
    "grandparent_pmcat_id": null,
    "all_pmcat_ids": ["39182"],
    "siblings": [
      {"mcat_id": "...", "mcat_name": "...", "listing_count": "...",
       "source_pmcat": "39182", "sibling_type": "direct | indirect"}
    ],
    "direct_sibling_count": 16,
    "indirect_sibling_count": 0
  },

  "candidate_summary": {
    "total": 81, "multi_source": 4,
    "structural_direct": 16, "structural_indirect": 0,
    "overlap": 0, "platform": 3, "semantic": 65
  },

  "candidates": [
    {
      "mcat_id": "81308",
      "mcat_name": "Portable Swimming Pools",
      "mcat_level": "4",
      "listing_count": "735",
      "source_flags": ["structural_direct", "overlap", "platform"],
      "source_tier_primary": "structural_direct",
      "pre_relevance": "high",
      "pre_relationship": "likely_sibling",
      "similarity_score": 1.0,
      "pre_note": "Direct sibling — PMCAT 39182",
      "overlap_data": {
        "total_products": 735, "total_suppliers": 312,
        "common_products": 187, "common_suppliers": 94,
        "product_overlap_pct_root": 14.8, "product_overlap_pct_candidate": 25.4,
        "supplier_overlap_pct_root": 22.1, "supplier_overlap_pct_candidate": 30.1
      }
    }
  ],

  "family_context_pairs": [
    {
      "mcat_a": "Portable Swimming Pools", "mcat_b": "Kids Swimming Pool",
      "common_products": 312, "common_suppliers": 143,
      "product_overlap_pct_a": 31.4, "product_overlap_pct_b": 15.4,
      "supplier_overlap_pct_a": 27.6, "supplier_overlap_pct_b": 14.9
    }
  ]
}
```

`overlap_data` is `null` for candidates where no overlap row exists — Agent 4 sets all overlap fields to 0 in that case.
`family_context_pairs` contains only sibling-to-sibling overlap pairs (Source C). Does NOT add new candidates. Agent 4 uses these to write `family_context_note` per candidate.

---

### Taxonomy Context Fields Used by Downstream Agents

| Agent | Fields consumed from taxonomy_context | Purpose |
|---|---|---|
| Agent 3 | `full_path`, `mcat_level`, `listing_count` | Anchor market research to correct domain; calibrate depth of research |
| Agent 4 | Full taxonomy_context | Already in agent4_input.json — no additional pass-through needed |
| Agent 8 | `mcat_level`, `listing_count` | Depth-calibrated nature classification; hard number for is_thin |
| Agent 12 | `siblings[]` (full name list) | Complete sibling naming audit; segment contamination detection |
| Agent 14 | `full_path`, `mcat_level`, `siblings[]`, `direct_sibling_count` | Reposition signal; intermediate node signal; naming inconsistency |

The orchestrator passes `taxonomy_context` to Agent 3 at Wave 1 start. Agent 3 appends it to `vision_output`. All Wave 2+ agents read it from there.
---
...
| `related_mcats[].reason` for cross_sell / accessory / parts / service | 1 sentence maximum. |
| `structural_signals.reposition_signal.issue` | 1 sentence — what is inconsistent. |
| `structural_signals.intermediate_node_signal` summary | 1 sentence per candidate label. |
...
**Inputs:** `mcat_name`, `mcat_id`, `product_page_url`, internal KWs, google KWs, `agent1_pdf_supplement` (may be null — identified-category products only, pre-filtered by Agent 1), `seller_buyer_call_data` rows for this MCAT (may be empty), `taxonomy_context` object from `agent4_input.json` (passed by orchestrator)
...
| `taxonomy_context` | Passed through unchanged from agent4_input.json — appended to vision_output so all Wave 2+ agents can read it |
...
**Inputs:**
- `vision_output` (Agent 3 — includes `taxonomy_context`)
- `agent4_input.json` for THIS MCAT — pre-built by the preprocessor

**The candidate pool, overlap data, mcat_level, listing_count, and family_context_pairs are fully pre-assembled in `agent4_input.json`. Agent 4 does NOT read any raw CSV files.**

---

**Step 1 — Read the pre-built candidate pool**

Load `candidates[]` from `agent4_input.json`. Each entry contains:
- `mcat_id`, `mcat_name`
- `mcat_level` — tree depth (null if not in tree)
- `listing_count` — active listings (null if not in tree)
- `source_flags` — which tiers found this candidate: `structural_direct`, `structural_indirect`, `overlap`, `platform`, `semantic`
- `source_tier_primary` — primary tier
- `pre_relevance`, `pre_relationship`, `similarity_score`, `pre_note`
- `overlap_data` — null if no overlap row exists; otherwise contains `total_products`, `total_suppliers`, `common_products`, `common_suppliers`, `product_overlap_pct_root`, `product_overlap_pct_candidate`, `supplier_overlap_pct_root`, `supplier_overlap_pct_candidate`

Use `source_flags` to understand each candidate's provenance — candidates with `structural_direct` are guaranteed tree siblings; `platform` candidates are existing platform links to audit; `semantic` candidates are keyword matches.

Use `mcat_level` to resolve parent vs sibling vs child with certainty: a candidate at a lower level number than root is a parent or super_parent; same level is a sibling; higher level number is a child.

Where `overlap_data` is null: set all overlap fields to 0 in the output.

---

**Step 2 — Build family context note per candidate**

Read `family_context_pairs[]` from `agent4_input.json`. These are sibling-to-sibling overlap pairs (neither is the root). Use them to write a `family_context_note` of 1 sentence per candidate, summarising how this candidate relates to other family members — e.g. "High overlap with Kids Swimming Pool (31.4%) and Readymade Swimming Pools (18.2%) — suggests near-duplicate cluster in portable entry-level segment."

Do NOT add new candidates from this step. `family_context_pairs` is context only.
...
**Output:** Full `related_mcats` list + slim output (name, mcat_id, relationship, sibling_bifurcation_type, is_duplicate, to_be_merged, is_brand_mcat, merge_signals_fired, category_nature.is_thin, category_nature.is_vague, source_flags, mcat_level, listing_count)
...
**Inputs:** `vision_output` (Agent 3 — includes `taxonomy_context.mcat_level` and `taxonomy_context.listing_count`)
...
**Depth calibration rules (use taxonomy_context):**
- `is_thin`: use `taxonomy_context.listing_count` as a hard anchor. `is_thin` is about whether the category justifies its own navigation node (buyer intent, search demand) — NOT listing count alone. But a hard listing count prevents estimating from page tiles.
- `is_generic`: a contradiction flag — a deep-level MCAT (level 5+) classified `is_generic: yes` is almost certainly wrong. Flag for review.
- `is_specific`: depth ≥ 4 generally supports `is_specific: yes` unless the name is genuinely vague.
...
**Inputs:** `vision_output` (Agent 3 — keyword data and `taxonomy_context.siblings`), Agent 4 slim output
...
**Step 2b — Naming Inconsistency Audit (new):**
Using `taxonomy_context.siblings` from vision_output, examine the naming pattern of the full structural sibling set.
- Identify the dominant naming convention (e.g. "Generator [Component]", "[Material] Pipes", "[Type] Swimming Pool").
- Flag any sibling (including root) that deviates from the dominant pattern.
- Brand MCATss (`is_brand_mcat: yes` from Agent 4) are exempt.
- Service MCATss (`is_service: yes` from Agent 8) are exempt.
- Output: `naming_inconsistency_flag` — fires when ≥1 deviation found.
...
**Output fields:**
`enhanced_name_4w/5w/6w/long`, `top_10_alt_names`, `segment_contamination_active`, `segment_contamination_dimensions`, `segment_contamination_disclaimer`, `alias_collision_merge_signals`, `naming_inconsistency_flag`
...
`naming_inconsistency_flag` format:
```json
{
  "fires": true,
  "dominant_pattern": "Generator [Component]",
  "deviating_nodes": [
    {"mcat_id": "", "mcat_name": "", "suggested_rename": ""}
  ],
  "exempt_nodes": [
    {"mcat_id": "", "mcat_name": "", "exempt_reason": "brand_mcat | service_mcat"}
  ]
}
```
...
**Inputs:** `vision_output` (Agent 3 — includes `taxonomy_context`), Agent 4 slim output, Agent 8 `category_nature`, Agent 12 `alias_collision_merge_signals`, Agent 12 `naming_inconsistency_flag`
...
**New Verdict Fields (from taxonomy_context):**

**`reposition_signal`** — Fires when root's current tree path appears structurally inconsistent with its classification or candidate relationships.
- Use `taxonomy_context.full_path` as the current path.
- Fires when: root is classified as `sibling_category` of a candidate that is at a different level, OR root's parent PMCAT is a broad catch-all while a more specific branch exists in the family.
- Confidence: always `MEDIUM` or `LOW`. Never auto-approved. Never assert a specific target path — only flag the inconsistency and describe the evidence.
- Format: `{fires: bool, current_path: str, issue: str, confidence: "MEDIUM | LOW"}`

**`intermediate_node_signal`** — Fires when root's parent PMCAT has too many direct children with no intermediate grouping.
- Threshold: `direct_sibling_count` ≥ 15 under the same immediate PMCAT.
- Suggest a grouping axis based on the sibling names (material, application, form_factor, segment).
- Format: `{fires: bool, parent_name: str, sibling_count: int, suggested_grouping_axis: str, candidate_intermediate_labels: [str]}`

**`naming_inconsistency_flag`** — Pass through from Agent 12 output unchanged. Written directly into structural_verdict.
...
**Rule 0b — Taxonomy header fields:** Write `full_path`, `mcat_level`, `listing_count` from taxonomy_context to both context.json and related_mcat_context.json root headers.
...
1. Root MCAT identity header — include `full_path`, `mcat_level`, `listing_count` from taxonomy_context.
2. `related_mcats[]` — full Agent 4 output for all candidates **except** those classified as `unrelated`. Include `mcat_level`, `listing_count`, `source_flags` per entry from Agent 4 slim output.
   - `reason` field: 1 sentence max for `cross_sell`, `accessory`, `parts`, `service`. 2-bullet justification for `sibling_category`, `parent_category`, `super_parent`, `substitute`, and any `is_duplicate: yes` entry.
   - `routing_note` field: write ONLY when `relationship` ∈ {sibling_category, parent_category, super_parent, substitute}. Omit for all other relationship types.
3. `unrelated_mcats[]` — flat list of mcat_names for all `unrelated` candidates. Names only.
4. `structural_signals` — from Agent 14: `reposition_signal`, `intermediate_node_signal`, `naming_inconsistency_flag`.
5. `merge_summary` — from Agent 14 `structural_verdict.mcats_to_absorb`.
6. `alias_collision_merge_signals` — from Agent 12.
...
```json
{
  ...
  "page_url": "",
  "full_path": "Group > Subcat > PMCAT > MCAT Name",
  "mcat_level": 4,
  "listing_count": 0,
  ...
  "structural_verdict": {
    ...
    "intermediate_node_signal": {
      "fires": false,
      "parent_name": "",
      "sibling_count": 0,
      "suggested_grouping_axis": "",
      "candidate_intermediate_labels": []
    },
    "naming_inconsistency_flag": {
      "fires": false,
      "dominant_pattern": "",
      "deviating_nodes": [],
      "exempt_nodes": []
    },
    "summary": "Max 5 bullets: (1) dissolve/rename/merge verdict, (2) each absorb candidate with signals + navigation loss, (3) proceed_to_cleaning gate, (4) reposition/intermediate/naming signals if fired."
  },
  ...
  "related_mcats": [
    {
      ...
      "mcat_level": 4,
      "listing_count": 0,
      "source_flags": ["structural_direct | structural_indirect | overlap | platform | semantic"],
      ...
      "reason": "sibling/parent/super_parent/substitute/is_duplicate:yes → 2-bullet justification. cross_sell/accessory/parts/service → 1-sentence max.",
      "family_context_note": "1 sentence on how this candidate relates to other siblings based on family_context_pairs.",
      ...
      "routing_note": "ONLY written when relationship ∈ {sibling_category, parent_category, super_parent, substitute}. Omit for all other types."
    }
  ],
