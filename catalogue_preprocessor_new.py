...
def build_taxonomy_context(mcat_id: str, mcat_info: dict,
    ...
    return {
        ...
        "is_orphan":      info.get("is_orphan", False) or None,
...
def extract_overlap_data(root_name: str, other_name: str,
    ...
    if m1.lower() == root_name.lower():
        return {
            ...
            "common_products":              safe_int(row.get("Common Product")),
            "common_suppliers":             safe_int(row.get("Common Suppliers")),
            "product_overlap_pct_root":     safe_float(row.get("(M1.M2)/M1 (Product)")),
            "product_overlap_pct_candidate": safe_float(row.get("(M1.M2)/M2 (Product)")),
            "supplier_overlap_pct_root":    safe_float(row.get("(M1.M2)/M1 (Supplier)")),
            "supplier_overlap_pct_candidate": safe_float(row.get("(M1.M2)/M2 (Supplier)")),
        }
    else:
        return {
            ...
            "common_products":              safe_int(row.get("Common Product")),
            "common_suppliers":             safe_int(row.get("Common Suppliers")),
            "product_overlap_pct_root":     safe_float(row.get("(M1.M2)/M2 (Product)")),
            "product_overlap_pct_candidate": safe_float(row.get("(M1.M2)/M1 (Product)")),
            "supplier_overlap_pct_root":    safe_float(row.get("(M1.M2)/M2 (Supplier)")),
            "supplier_overlap_pct_candidate": safe_float(row.get("(M1.M2)/M1 (Supplier)")),
        }
...
def build_family_context_pairs(sibling_names: set[str],
    ...
    for row in all_pairs:
        ...
        if m1.lower() in sib_lower and m2.lower() in sib_lower:
            ...
            cp = si(row.get("Common Product"))
            cs = si(row.get("Common Suppliers"))
            
            # Only provide the family context pairs data if there are common products or suppliers
            if (cp and cp > 0) or (cs and cs > 0):
                result.append({
                    "mcat_a": m1,
                    "mcat_b": m2,
                    "common_products":       cp,
                    "common_suppliers":      cs,
                    "product_overlap_pct_a": sf(row.get("(M1.M2)/M1 (Product)")),
                    "product_overlap_pct_b": sf(row.get("(M1.M2)/M2 (Product)")),
                    "supplier_overlap_pct_a": sf(row.get("(M1.M2)/M1 (Supplier)")),
                    "supplier_overlap_pct_b": sf(row.get("(M1.M2)/M2 (Supplier)")),
                })
    return result
...
def build_candidate_pool(
    ...
    def upsert(mcat_id: str, mcat_name: str, tier: str,
        ...
        if mcat_id not in pool:
            ...
            pool[mcat_id] = {
                "mcat_id":             mcat_id,
                "mcat_name":           mcat_name,
                "mcat_level":          mcat_level,
                "listing_count":       listing_count,
                "source_flags":        [],
                "relevance":           pre_relevance,
                "rel_type":            pre_relationship,
                "score":               round(similarity_score, 3) if similarity_score is not None else None,
                "note":                pre_note,
                "overlap_data":        None,
            }
        entry = pool[mcat_id]
        ...
        if overlap_data and entry["overlap_data"] is None:
            # Only keep non-null fields in overlap_data to save space
            entry["overlap_data"] = {k: v for k, v in overlap_data.items() if v is not None}
    ...
    for sib in taxonomy_ctx.get("siblings", []):
        if sib.get("sibling_type") == "direct":
            upsert(sib["mcat_id"], sib["mcat_name"],
                   "structural_direct", "high", "sibling",
    ...
    for sib in taxonomy_ctx.get("siblings", []):
        if sib.get("sibling_type") == "indirect":
            upsert(sib["mcat_id"], sib["mcat_name"],
                   "structural_indirect", "high", "sibling",
                   1.0, f"Indirect sibling (GP PMCAT {sib['source_pmcat']})")
...
def run(root_mcat_id: str, root_mcat_name: str,
    ...
    # Remove siblings from taxonomy_context as requested (already in candidates)
    ctx.pop("siblings", None)
    ctx.pop("direct_sibling_count", None)
    ctx.pop("indirect_sibling_count", None)
    ...
    # Final assembly with null pruning
    def prune(obj):
    ...
    output = prune({
        ...
        "family_context_pairs": fcp if fcp else None,
    })
